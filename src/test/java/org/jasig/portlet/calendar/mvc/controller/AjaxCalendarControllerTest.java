...
public class AjaxCalendarControllerTest {
    ...
    private Set<CalendarDisplayEvent> calendarDisplayEvents;
	...
	public void setUp() {
        ...
        session.setAttribute("timezone", "America/Phoenix");
		nonEmptyMap = new HashMap<String, Object>();
		nonEmptyMap.put("aaa", new Object());
		modelEtag = ""+nonEmptyMap.hashCode();
		request.setResourceID(partialWellFormedResourceID + "-220108613");
		ReflectionTestUtils.setField(testee, "helper", mockHelper);

        calendarDisplayEvents = new HashSet<CalendarDisplayEvent>();
        calendarDisplayEvents.add(createLongCalendarDisplayEvent());
	}

    private CalendarDisplayEvent createLongCalendarDisplayEvent() {
        ...
        Interval eventInterval = new Interval(start.getTime(), end.getTime());
        Interval dayInterval = new Interval(start.getTime(), start.getTime() + (1000 * 60 * 60 * 24));
        CalendarDisplayEvent calendarDisplayEvent = new CalendarDisplayEvent(event, eventInterval, dayInterval,
                ISODateTimeFormat.basicDate(), ISODateTimeFormat.basicDate());
        return calendarDisplayEvent;
    }

	@Test
	public void testWhenRequestEtagMatchesModelEtagThenEmptyModelAndViewReturned() throws Exception{
		when(mockHelper.getEventList(any(List.class), any(Interval.class), eq(request))).thenReturn(calendarDisplayEvents);
